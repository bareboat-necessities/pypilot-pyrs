...
";
    ...
    fn fittext<T0, T1, T2, T3, RT>(&self, rect: T0, text: T1, wordwrap: T2, fill: T3) -> RT {
        ...
        if fill != "none" {
            surface.
            box (starred!((self.convrect(rect) + vec![fill]))/*unsupported*/);
        }
        let metric_size = 16;
        let mut ptext = text;
    ...
    fn rectangle<T0, T1>(&self, rect: T0, width: T1) {
        ...
        if !width {
            surface.
            box (starred!((self.convrect(rect) + vec![white]))/*unsupported*/);
        } else {
            let
...
impl controlbase {
    ...
    fn display<T0>(&self, refresh: T0) {
        if refresh {
            self.
            box (rectangle(0, 0.92, 1, 0.1), black);
            self.wifi = false;
            self.pilot = false;
        }
        if self.lcd.battery_voltage {
            ...
            if batt != self.batt || refresh {
                self.batt = batt;
                self.lcd.surface.
                box (starred!((self.convrect(battrect) + vec![black]))/*unsupported*/);
        ...
        if wifi {
            ...
            self.lcd.surface.
            box (starred!((self.convrect(wifirect) + vec![black]))/*unsupported*/);
        }
    }
}
...
impl control {
    ...
    fn display_mode(&self) {
        ...
        let marg = 0.02;
        self.lcd.surface.
        box (starred!((self.convrect(rectangle(0, 0.74, 1, (0.16 + marg))) + vec![black]))/*unsupported*/);
        for mode in modes {
            if modes[mode][1]() {
    ...
    fn display<T0, RT>(&self, refresh: T0) -> RT {
        ...
        fn draw_big_number<T0, T1, T2>(pos: T0, num: T1, lastnum: T2) {
            ...
            for i in (0..3) {
                ...
                let x = (float(i) * 0.33);
                self.
                box (rectangle(x, pos, 0.34, 0.4), black);
        ...
        fn draw_heading<T0, T1, T2>(pos: T0, value: T1, lastvalue: T2) {
            ...
            if windmode {
                if heading > 0 {
                    self.
                    box (rectangle(0.7, (pos + 0.3), 0.3, 0.025), white);
                } else {
                    if heading < 0 {
                        self.
                        box (rectangle(0, (pos + 0.3), 0.3, 0.025), white);
                    }
                }
            }
        }
